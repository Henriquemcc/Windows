...
while ($opcao -ne 0) {
    ...
    if ($opcao -eq 1) {
        & ([System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($MyInvocation.MyCommand.Definition), "Menu", "Instalar.ps1"))
    }
    ...
    elseif ($opcao -eq 7) {
        & ([System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($MyInvocation.MyCommand.Definition), "Menu", "Parar.ps1"))
    }
