...
$numeroDisco = Read-Host -Prompt "Digite o número do disco que deseja montar: "
try {
...
$confirmacao = Read-Host -Prompt "Tem certeza que deseja montar o disco $($discos[$numeroDisco])? (s/n)"
if ($confirmacao.ToLower()[0] -eq "s") {
    wsl --mount $($discos[$numeroDisco].DeviceID)
}