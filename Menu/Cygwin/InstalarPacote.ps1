...
function AdicionarPacote {
    ...
    $confirmacao = Read-Char -Prompt "Deseja adicionar o pacote `"$($nomePacote)`" a lista de pacotes a serem instalados? (s/n) "
...
function RemoverPacote {
    try {
        ...
        $confirmacao = Read-Char -Prompt "Deseja remover o pacote `"$($nomePacote)`" da lista de pacotes a serem instalados? (s/n) "
        if ($confirmacao -eq "s" -or $confirmacao -eq "S") {
...
function Aplicar {
    ...
    try {
        if ($pacotes.Count -eq 0) {
            throw "Não há pacotes a serem instalados."
        }
...
function ObterOpcao {
    ...
    while ($_opcao -lt 0 -or $_opcao -gt 4) {
        ...
        Write-Host -Object "1 - Listar pacotes da lista de pacotes a serem instalados"
        Write-Host -Object "2 - Adicionar pacote a lista de pacotes a serem instalados"
        Write-Host -Object "3 - Remover pacote da lista de pacotes a serem instalados"
...
Write-Host -Object "Menu de instalação de pacotes Cygwin"
